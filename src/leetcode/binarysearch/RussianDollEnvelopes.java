...
//		int[][] envelopes = {{1,3},{3,5},{6,7},{6,8},{8,4},{9,5}};
	...
	/** 
	 * 320 ms, faster than 42.57%
	 * By Longest Increasing Subsequence, O(n^2)
	 */
	public static int maxEnvelopes_LisOn2(int[][] envelopes) {
		if( envelopes==null || envelopes.length==0 )
			return 0;
	...
	/** Time Limit Exceeded */
	public static int maxEnvelopes_暴力遞迴(int[][] envelopes) {
		if( envelopes==null || envelopes.length==0 )
			return 0;
		
		final boolean isWleH = isWleH(envelopes);
		final int w = isWleH?0:1;
		final int h = isWleH?1:0;
		
		Arrays.sort(envelopes, new Comparator<int[]>() {
			@Override
			public int compare(int[] o1, int[] o2) {
				int i = o1[w]-o2[w];
				return i==0?o1[h]-o2[h]:i;
			}
		});
//		System.out.println(Arrays.stream(envelopes).map(Arrays::toString).collect(Collectors.joining(", ")));
		
		return maxEnvelopes(envelopes, 0, new int[] {0,0}, 0);
	}

	public static int maxEnvelopes(int[][] envelopes, int from, int[] last, int count) {
		if( from==envelopes.length )
			return count;
		if( envelopes[from][0]<=last[0] || envelopes[from][1]<=last[1] )
			return maxEnvelopes(envelopes, from+1, last, count);
			
		int hasCur = maxEnvelopes(envelopes, from+1, envelopes[from], count+1);
		int noCur = maxEnvelopes(envelopes, from+1, last, count);
		
//		System.out.println("from="+from+", last="+Arrays.toString(last)+", noCur="+noCur+", hasCur="+hasCur+", count="+count);
		return Math.max(noCur, hasCur);
	}
	
	public static boolean isWleH(int[][] envelopes) {
